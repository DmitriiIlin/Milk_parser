...
class Perekrestok_milk_parser:
    #Класс содержащий базовую информацию 
    def __init__(self):
        self.base_link = "https://www.perekrestok.ru"
        self.path_to_file = 'C:\\Users\\Дмитрий\\AppData\\Local\\Programs\\Python\\Milk_parser'
        self.name = "Milk_parser.xlsx"
        self.folder_name = "Milk_parser"


    def get_all_milk_links(self, target = "https://www.perekrestok.ru/catalog/moloko-syr-yaytsa/moloko?page=1"):
            ...
            if response.status_code == 200:
                ...
                if len(base_blocks) == 0:
                    ...
                    for div in base_blocks:
                        ...
                        full_link = self.base_link + base_div_2
    ...
    def get_information_from_link (self, link):
        ...
        if response.status_code == 200:
            ...
            picture_link = self.base_link + (str(picture))
    ...
    def create_excel_doc (self):
        ...
        if not os.path.isdir(self.folder_name): #Создае там папку , если ее нет, с именем folder_name
    ...
    def get_picture_for_excel_file(self, input_information, size=(100, 80)):
    ...
    def load_information_to_excel_file(self, work_sheet_name, all_milk_data, size = (100, 100), start_column = 1, start_row = 1):
        ...
        for i in range(0, q_ty_of_milk):
            ...
            if milk_information != 0:
                ...
                img = openpyxl.drawing.image.Image(self.get_picture_for_excel_file(milk_picture))
    ...
    def open_excel_and_put_information_to_file(self):
...
Excel_Book=Perekrestok_milk_parser()
#Основная программа
...
sheet_name = Excel_Book.open_excel_and_put_information_to_file() 
    #Блок_2: Получаем все ссылки на молоко
all_links = Excel_Book.get_all_milk_links()
...
for i in range(0, links_q_ty):
    ...
    milk_data =Excel_Book.get_information_from_link(all_links[i])  
    all_milk_data.append(milk_data)
    #print(all_milk_data)
    #Блок_3: Записываем полученную информацию     
Excel_Book.load_information_to_excel_file(sheet_name, all_milk_data) 
